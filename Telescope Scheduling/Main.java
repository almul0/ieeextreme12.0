...
class Main {
    public static void main (String[] args) throws java.lang.Exception {
        ...
        int N = in.nextInt();
        IntervalTree<Star> it = new IntervalTree<>();
        ArrayList<Star> stars = new ArrayList<>(N);
        Star s;
        for (int i = 0; i < N; i++) {
            s = new Star(in.nextInt(), in.nextInt(),in.nextInt(), i+1);
            it.addInterval(s.start, s.end, s);
            stars.add(s);
        }
        List<Star> result;

        for (Star si: stars) {
             result = it.get(si.start);
             System.out.printf("Star %d is conflict with ",si.id);
             for (Star sr: result) {
                 System.out.printf("%d, ", sr.id);
             }
             System.out.println();
        }
...
class Star{
    ...
    int des;
    int id;
    public Star(int start, int end, int des, int id){
        this.id = id;
        this.start = start;
        this.end = end;
        this.des = des;
